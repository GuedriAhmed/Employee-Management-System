...
try{
    ...
    role,
     ...
     if (user){
        return res.status(400).json({succes : false ,error :"user already registred in emp"})
     }
...
const updateEmployee= async (req,res) =>{
  try{
    ...
    salary,
     ...
     if (!employee){return res.status(404).json({succes:false,error:"employee not found"})
    }
  const user = await User.findById({_id: employee.userId})
  if (!user){return res.status(404).json({succes:false,error:"user not found"})
    }
  ...
  if(!updateEmployee ||!updateUser){
    return res.status(404).json({ success: false, error: 'Server error updating employees' });
  }
